...
class AToB(DirectRLEnv):
    ...
    def __init__(self, cfg: AToBCfg, render_mode: str | None = None, **kwargs):
        self.debug = False  
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        for object_name in self.object_names:
            ...
            if object_name == "asd":
                ...
                random = torch.zeros((len(env_ids),3),device=self.device)
            object_default_state[:, 0:3] = (
                object_default_state[:, 0:3] + self.scene.env_origins[env_ids] + random
            ) 
            ...
            if object_name == "alphabet_soup":
                self.target_pos[env_ids] = torch.tensor([0,0,0.3],device=self.device) + object_default_state[:, 0:3]
    ...
    def _get_observations(self) -> dict:
        ...
        self.target_to_hand_pos = self.target_pos - self.robot_grasp_pos # n, 3
